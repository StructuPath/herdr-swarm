...
// abort/prune make no commits themselves, but prune's revert scenario and the
// repo fixtures do; identity must ride in explicitly (GIT_CONFIG_GLOBAL is
// /dev/null in the harness env).
const gitIdent = {
...
// A fanned-out-looking run, built the same way harvest.test.mjs builds one:
// real repo, real slot worktrees on run-unique branches at the recorded fork
// SHA, manifest shaped exactly like fanout-pane.sh writes it.
function mkRun(opts = {}) {
	const nslots = opts.slots ?? 1;
	const repo = h.makeRepo();
	const runId = `r-cl${++runSeq}`;
...
test("dirty slot worktree is KEPT and reported — never prompted, never forced", () => {
	...
	// The manifest is archived either way; the kept slot keeps its status so
	// the recovery record says what was left behind.
	assert.equal(r.archived().slots[0].status, "running");
});
...
test("prune with confirm deletes merged branch and backup ref; unmerged and foreign branches survive", () => {
	...
	assert.equal(
		h.git(p.repo, "for-each-ref", "refs/swarm-backups").stdout.trim(),
		"",
		"backup ref deleted under confirm",
	);
	assert.match(r.stdout, /prune summary/);
});
